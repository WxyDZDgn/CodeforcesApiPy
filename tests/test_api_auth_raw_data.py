"""
Testing requests to api with auth in raw data.
"""
...
def test_blog_entry_comments(api_key, api_secret):
    api = CodeforcesApi(api_key, api_secret, raw_data=True)
...
def test_recent_status(api_key, api_secret, check_problem, check_party):
    api = CodeforcesApi(api_key, api_secret, raw_data=True)
...
def test_recent_actions(api_key, api_secret, check_blog_entry, check_comment):
    api = CodeforcesApi(api_key, api_secret, raw_data=True)
...
def test_user_friends(api_key, api_secret):
    ...
    assert "aropan" in friends or "gepardo" in friends or "acacachhh" in friends
...
def test_user_status(api_key, api_secret, check_problem, check_party):
    api = CodeforcesApi(api_key, api_secret, raw_data=True)
